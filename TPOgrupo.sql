...
----------------------------------------------------------
--Devolver todos los Carpinteros que trabajen full time
----------------------------------------------------------
...
where DisponibilidadHoraria = 'Full time' and o.NombreOficio = 'Carpintero';
...
(SELECT COUNT(S.IDsolicitud)
 FROM SolicitudServicio s
 WHERE s.IDCliente = c.IDCliente) 
 as CantidadSolicitudes
From cliente c 
...
WHERE P.PromedioCalificacion = 5 AND EXISTS(
SELECT PromedioCalificacion 
FROM Cliente c
where c.PromedioCalificacion = 5);
